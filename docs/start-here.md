...
**Lab 02 route:** [Step 1 setup/design](../.github/steps/01.md) → [Step 2 network/Actions authoring](../.github/steps/02.md) → [Step 3 outputs/checks](../.github/steps/03.md) → [Step 4 PR/Azure inspection/update/cleanup](../.github/steps/04.md). The actions are embedded in those four lessons, not a separate activity.
...
Create Actions inside [Step 2](../.github/steps/02.md), run the inline [Step 3 checks](../.github/steps/03.md), then continue [Step 4](../.github/steps/04.md), even after automatic 4/4 advancement. Step 4 contains the STOP conditions, passing-PR merge, portal clicks, tag update, followup and separately authorized cleanup.

Keep `WORKSHOP_AZURE_ENABLED=false` until the owner verifies readiness. Missing prerequisites mean **live continuation pending**, not permission to create main or bypass controls. [Owner configuration](delivery-configuration.md) is a reference, not a fifth lesson. No live result is claimed.
