...
import approve from "../scripts/avm-approval.cjs";
...
function approvalFixture() {
  ...
  const author = { id: 101, login: "unit-author", type: "User" };
  const reviewer = { id: 303, login: "unit-reviewer", type: "User" };
  const context = { repo: { owner: "example", repo: "private-copy" }, sha: binding.sha, runId: 123, actor: "unit-context-actor" };
  const state = {
    branch: { protected: true, commit: { sha: binding.sha } },
    run: { actor: { id: 201, login: "unit-run-actor", type: "User" }, triggering_actor: { id: 202, login: "unit-triggering-actor", type: "User" } },
    prs: [{ number: 7, merged_at: "2026-09-21T00:00:00Z", base: { ref: "main" }, merge_commit_sha: binding.sha, user: author }],
    approvals: [{ state: "approved", environments: [{ id: 77, name: "avm-apply" }], user: reviewer }],
    environment: { deployment_branch_policy: { protected_branches: false, custom_branch_policies: true }, can_admins_bypass: false, protection_rules: [{ type: "required_reviewers", prevent_self_review: true, reviewers: [{ type: "User", reviewer }] }] },
    branches: [{ id: 88, name: "main", type: "branch" }],
  };
  const calls = [], outputs = [], summaries = [];
  const expectArgs = (actual, fields) => assert.deepEqual(actual, { ...context.repo, ...fields });
  const listPrs = () => assert.fail("Use the mocked pagination boundary");
  const listBranches = () => assert.fail("Use the mocked pagination boundary");
  const github = {
    rest: {
      repos: {
        getBranch: async (args) => { expectArgs(args, { branch: "main" }); calls.push("branch"); return { data: structuredClone(state.branch) }; },
        getEnvironment: async (args) => { expectArgs(args, { environment_name: "avm-apply" }); calls.push("environment"); return { data: structuredClone(state.environment) }; },
        listPullRequestsAssociatedWithCommit: listPrs, listDeploymentBranchPolicies: listBranches,
      },
      actions: { getWorkflowRun: async (args) => { expectArgs(args, { run_id: context.runId }); calls.push("run"); return { data: structuredClone(state.run) }; } },
    },
    paginate: async (route, args) => {
      if (route === listPrs) { expectArgs(args, { commit_sha: context.sha, per_page: 100 }); calls.push("prs"); return structuredClone(state.prs); }
      if (route === listBranches) { expectArgs(args, { environment_name: "avm-apply", per_page: 100 }); calls.push("branches"); return structuredClone(state.branches); }
      assert.equal(route, "GET /repos/{owner}/{repo}/actions/runs/{run_id}/approvals", "No live or mutating GitHub client exists in this fixture");
      expectArgs(args, { run_id: context.runId, per_page: 100 }); calls.push("approvals"); return structuredClone(state.approvals);
    },
  };
  const summary = {
  ...
  const core = { setOutput: (name, value) => outputs.push([name, value]), summary };
  return { state, calls, outputs, summaries, args: { github, context, core } };
}

test("independent approval verification uses only a mocked read-only GitHub client", { concurrency: false }, async (t) => {
  const previousAttempt = process.env.GITHUB_RUN_ATTEMPT, previousCwd = process.cwd();
  t.after(() => {
  ...
  // The current implementation imports policy relative to cwd; no state or
  // module cache is consulted, and the caller's directory is always restored.
  process.chdir(fileURLToPath(new URL("../", import.meta.url)));
  t.mock.method(globalThis, "fetch", async () => { throw new Error("External HTTP is forbidden in this test"); });
  const rejected = async (sample, pattern) => {
    await assert.rejects(approve(sample.args), pattern);
    assert.deepEqual(sample.outputs, [], "Rejected mock approvals must not emit an apply binding");
    assert.deepEqual(sample.summaries, [], "Rejected mock approvals must not claim verified approval");
  };

  await t.test("a distinct human on a protected current merged-main commit passes the pure verification function", async () => {
    const sample = approvalFixture();
    await approve(sample.args);
    assert.deepEqual(sample.outputs, [["main_sha", sample.args.context.sha]]);
    assert.deepEqual(sample.calls, ["branch", "run", "prs", "approvals", "environment", "branches"]);
    assert.equal(sample.summaries.filter(([kind]) => kind === "write").length, 1);
  });

  await t.test("every associated PR author is excluded by immutable user ID, even after renaming", async () => {
    for (const secondAuthor of [false, true]) {
      const sample = approvalFixture();
      if (secondAuthor) sample.state.prs.push({ ...structuredClone(sample.state.prs[0]), number: 8, user: { id: 102, login: "unit-second-author", type: "User" } });
      sample.state.approvals[0].user = { ...sample.state.prs.at(-1).user, login: "unit-renamed-author" };
      await rejected(sample, /independent avm-apply/);
    }
  });

  await t.test("run actor, triggering actor and event actor cannot self-approve", async () => {
    for (const who of ["actor", "triggering_actor", "context"]) {
      const sample = approvalFixture();
      sample.state.approvals[0].user = who === "context" ? { id: 204, login: sample.args.context.actor, type: "User" } : structuredClone(sample.state.run[who]);
      await rejected(sample, /independent avm-apply/);
    }
  });

  for (const who of ["actor", "triggering_actor"]) await t.test(`${who} remains excluded when its immutable ID matches but its login spelling changes`, async () => {
    const sample = approvalFixture();
    sample.state.approvals[0].user = { ...sample.state.run[who], login: sample.state.run[who].login.toUpperCase() };
    await rejected(sample, /independent avm-apply/);
  });

  await t.test("bot identities never qualify as independent human reviewers", async () => {
    const sample = approvalFixture();
    sample.state.approvals[0].user = { id: 404, login: "unit-reviewer[bot]", type: "Bot" };
    await rejected(sample, /independent avm-apply/);
  });

  await t.test("absent, rejected, wrong-environment and unidentified approvals fail closed", async () => {
    for (const mutate of [
      (state) => { state.approvals = []; },
      (state) => { state.approvals[0].state = "rejected"; },
      (state) => { state.approvals[0].environments = [{ name: "avm-plan" }]; },
      (state) => { delete state.approvals[0].user; },
    ]) {
      const sample = approvalFixture(); mutate(sample.state);
      await rejected(sample, /independent avm-apply/);
    }
  });

  await t.test("an exact merged-main PR is required, not just an associated or open PR", async () => {
    for (const mutate of [
      (state) => { state.prs = []; },
      (state) => { state.prs[0].merged_at = null; },
      (state) => { state.prs[0].base.ref = "dev"; },
      (state) => { state.prs[0].merge_commit_sha = "b".repeat(40); },
    ]) {
      const sample = approvalFixture(); mutate(sample.state);
      await rejected(sample, /reviewed merged-main commit/);
    }
  });

  await t.test("stale or unprotected main stops before approval history can be credited", async () => {
    for (const mutate of [(state) => { state.branch.commit.sha = "b".repeat(40); }, (state) => { state.branch.protected = false; }]) {
      const sample = approvalFixture(); mutate(sample.state);
      await rejected(sample, /Main moved or is unprotected/);
      assert.deepEqual(sample.calls, ["branch"]);
    }
  });

  await t.test("admin bypass, self-review, missing reviewers and broad branch policies cannot authorize apply", async () => {
    for (const mutate of [
      (state) => { state.environment.can_admins_bypass = true; },
      (state) => { state.environment.protection_rules[0].prevent_self_review = false; },
      (state) => { state.environment.protection_rules[0].reviewers = []; },
      (state) => { state.environment.protection_rules = []; },
      (state) => { state.environment.deployment_branch_policy = { protected_branches: true, custom_branch_policies: false }; },
      (state) => { state.branches[0].name = "*"; },
      (state) => { state.branches[0].type = "tag"; },
      (state) => { state.branches.push({ name: "dev", type: "branch" }); },
    ]) {
      const sample = approvalFixture(); mutate(sample.state);
      await rejected(sample, { code: "ERR_ASSERTION" });
    }
  });

  await t.test("attempt two is rejected before any mocked GitHub lookup", async (child) => {
    const before = process.env.GITHUB_RUN_ATTEMPT;
    child.after(() => { process.env.GITHUB_RUN_ATTEMPT = before; });
    process.env.GITHUB_RUN_ATTEMPT = "2";
    const sample = approvalFixture();
    await rejected(sample, /new plan and approval/);
